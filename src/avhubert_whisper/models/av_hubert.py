...
class AVHubertEncoderWrapper(nn.Module):
    def __init__(self, av_hubert_model=None, freeze=True, output_dim: int = 2048):
        ...
        if av_hubert_model is not None and freeze:
            for param in av_hubert_model.parameters():
                param.requires_grad = False
            
        # Projection layer from AV-HuBERT dim to target dim
        if av_hubert_model is not None:
            self.projection = nn.Linear(av_hubert_model.output_dim, output_dim)
            
            # Initialize the projection with a small variance to avoid vanishing/exploding gradients
            nn.init.normal_(self.projection.weight, mean=0.0, std=0.02)
            nn.init.zeros_(self.projection.bias)
            
            # Log the dimensions
            logging.info(f"Initialized AVHuBERTWrapper with projection from {av_hubert_model.output_dim} to {output_dim}")
        else:
            self.projection = None
            logging.warning("AVHuBERTWrapper initialized without AV-HuBERT model!")
    
    def forward(self, *args, **kwargs):
        """Forward pass through the encoder wrapper"""
        if self.av_hubert is None:
            raise ValueError("AV-HuBERT model is not initialized!")
            
        # Get features from AV-HuBERT
        with torch.set_grad_enabled(not self.freeze):
            features = self.av_hubert(*args, **kwargs)
