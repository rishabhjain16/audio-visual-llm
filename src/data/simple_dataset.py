...
class AVSRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Create a task description for speech recognition
        task_description = "Transcribe the speech from this audio-visual input."
...
def create_dataloaders(
    ...
    if len(dataset) == 0:
        logging.error(f"Dataset is empty after filtering")
        raise ValueError(f"Dataset is empty after filtering")
    
    # Create dataloader
    logging.info(f"Creating {split} dataloader with batch size {batch_size}...")
    
    # Disable pin_memory to avoid CUDA errors with mixed data types
    ...
    dataloader = DataLoader(
        ...
        drop_last=(split == "train"),  # Only drop last batch during training
        persistent_workers=(num_workers > 0),  # Keep workers alive between batches
    )
    
    logging.info(f"Created {split} dataloader with {len(dataset)} samples")
    return dataloader 